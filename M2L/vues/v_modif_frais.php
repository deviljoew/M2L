...
<div class="container-fluid">
  <div class="row">
    ...
    <?php
      ...
      <form onSubmit="return verifChamps();" action="index.php?uc=formulaire&motif=<?=$_REQUEST['motif']?>&trajet=<?=$_REQUEST['trajet']?>&km=<?=$_REQUEST['km']?>&peage=<?=$_REQUEST['peage']?>&repas=<?=$_REQUEST['repas']?>&hebergement=<?=$_REQUEST['hebergement']?>&action=misAjoursFrais" method="POST">
        <table class="table table-light table-hover table-bordered">
          ...
          <tr>
              <td class="display-4" style="font-size:20px;text-align:center;"><input type="text" disabled class="form-control form-control-sm" value="<?php echo $date?>" id="date" name="date" placeholder="Date" required></td>
