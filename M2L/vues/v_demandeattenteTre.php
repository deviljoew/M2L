    ...
    <div class="col-md-12" style="padding-bottom:30px;background-color:#00000055;opacity:0.9;">
      <p class="display-4" style="color:white;margin-left:20px;margin-top:10px;">Notes de frais à valider :</p>
          ...
          <tr>
           ...
           <td class="h4" style="font-size:17px;text-align:center;"><a href="index.php?uc=formulaire&date=<?=$date?>&adressemail=<?php echo $unfrais['ADRESSE_MAIL'] ?>&motif=<?=$unfrais['LIBELLE']?>&trajet=<?=$unfrais['TRAJET']?>&km=<?=$unfrais['KM']?>&peage=<?=$unfrais['COUT_PEAGE']?>&repas=<?=$unfrais['COUT_REPAS']?>&hebergement=<?=$unfrais['COUT_HEBERGEMENT']?>&action=validerFrais"><img width="30px" src="./images/valider.png"/></a></td>
            <td class="h4" style="font-size:17px;text-align:center;"><a href="index.php?uc=formulaire&date=<?=$unfrais['DATE']?>&action=supprimerfrais" onclick="return confirm('Voulez-vous vraiment refuser cette ligne de frais? Attention, aucun retour ne sera possible.');"><img width="30px" src="./images/refuser.png"/></a></td>
         </tr>
          <?php } ?>
        </table>
    </div>
